...
if [ "$CHOICE" -eq 1 ]; then
		...
		# 提取 server {} 块的内容，排除 location 和注释行
		server_block=$(awk '
			/^server {/,/^}/ {
				# 忽略 location 块的内容
				if ($0 ~ /^location/) { in_location=1 }
				# 忽略注释行
				if ($0 ~ /^#/) { next }
				# 如果不在 location 块内且不是注释行，则打印内容
				if (in_location == 0) print $0
				# 结束 location 块
				if ($0 ~ /^}/ && in_location == 1) { in_location=0 }
			}
		' "$temp_file")
		# 检查是否成功提取 server 块
...
echo "分享链接：vless://$UUID@$IP:$PORT?encryption=none&security=reality&sni=$DOMAIN&fp=chrome&pbk=$PUBLIC_KEY&sid=$SHORTID&type=grpc&authority=$DEST&serviceName=$SERVICE_NAME&mode=gun#test"
